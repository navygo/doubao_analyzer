...
void ConfigManager::parse_config(const nlohmann::json& config) {
    ...
    if (config.contains("database")) {
        ...
        if (db.contains("host")) db_config_.host = db["host"];
        if (db.contains("port")) db_config_.port = db["port"];
        if (db.contains("user")) db_config_.user = db["user"];
        if (db.contains("password")) db_config_.password = db["password"];
        if (db.contains("name")) db_config_.database = db["name"];
        if (db.contains("charset")) db_config_.charset = db["charset"];
        if (db.contains("connection_timeout")) db_config_.connection_timeout = db["connection_timeout"];
        if (db.contains("read_timeout")) db_config_.read_timeout = db["read_timeout"];
        if (db.contains("write_timeout")) db_config_.write_timeout = db["write_timeout"];
    }

    // 解析备份配置
    if (config.contains("backup")) {
        const auto& backup = config["backup"];
        if (backup.contains("auto_backup")) backup_config_.auto_backup = backup["auto_backup"];
        if (backup.contains("backup_interval_hours")) backup_config_.backup_interval_hours = backup["backup_interval_hours"];
        if (backup.contains("backup_retention_days")) backup_config_.backup_retention_days = backup["backup_retention_days"];
        if (backup.contains("backup_path")) backup_config_.backup_path = backup["backup_path"];
    }

    // 解析清理配置
    if (config.contains("cleanup")) {
        const auto& cleanup = config["cleanup"];
        if (cleanup.contains("auto_cleanup")) cleanup_config_.auto_cleanup = cleanup["auto_cleanup"];
        if (cleanup.contains("retention_days")) cleanup_config_.retention_days = cleanup["retention_days"];
    }
}
