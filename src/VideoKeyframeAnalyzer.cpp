...
VideoMetadata VideoKeyframeAnalyzer::get_video_metadata(const std::string& video_url) {
    ...
    try {
        ...
        if (json_result.contains("streams") && json_result["streams"].is_array() && 
            ...
            if (stream.contains("width")) metadata.width = stream["width"];
            if (stream.contains("height")) metadata.height = stream["height"];
            ...
            if (stream.contains("codec_name")) metadata.codec = stream["codec_name"];
